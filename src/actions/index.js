...
export function fetchCoords(address) {
  ...
  .then((json) => {
    formattedCoords = json.results[0].geometry.location;
...
export function fetchRoute(toPlace, fromPlace, departOrArrive, date, distance) {
  ...
  const toPlaceCoords = fetchCoords(formattedToPlace);
  const fromPlaceCoords = fetchCoords(formattedFromPlace);
  console.log(toPlaceCoords);
